...
function ChannelList(props: { location: RouteComponentProps<any, StaticContext, any>, game: Archive }) {
	function ChannelListSelection(prps: RouteComponentProps<any, StaticContext, any>) {
		...
		console.log(prps.match.params.cid);
		console.log(prps.match);
		return (
			<div className="channel-list-container">
				<Scroller>
					{props.game.channels.map((c, i) => (
...
function MessageGroup(props: { messages: Message[], users: any }) {
	...
	function MessageLine(props: { text: string }) {
		return (
			<div className="message-line">{props.text}</div>
		)
	}
				...
				if (i == 0) {
					...
					return (
						<div className="first-message" key={i}>
							...
							<MessageLine text={m.content} />
						</div>
					)
				}
				else {
					return <MessageLine text={m.content} key={i} />
				}
			})}
		</div>
	)
