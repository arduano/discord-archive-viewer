import { Switch, RouteComponentProps, StaticContext, Route } from "react-router";
			...
			<div className="channel-list-container">
				<Scroller>
					{props.game.categories.map((c, i) =>
						<Category key={i} name={c.name.toUpperCase()} id={c.id} channels={props.game.channels.filter(_c => _c.parentID == c.id)} location={props.location} />
					)}
				</Scroller>
			</div>
		)
...
function Category(props: { id: string, name: string, channels: Channel[], location: RouteComponentProps<any, StaticContext, any> }) {
	const [open, setOpen] = useState<boolean>(false);
	let channel = props.channels.find(c => c.name == props.location.match.params.cid);

	return (
		<div className="category-container">
			<div className="category-line" onClick={() => setOpen(!open)}>
				<div className="material-icons">{open ? "keyboard_arrow_down" : "keyboard_arrow_right"}</div>
				<div>{props.name}</div>
			</div>
			<div className={`${open ? '' : 'collapsed'}`}>
				{props.channels.map((c, i) => (
					<Link to={props.location.match.url + '/' + c.name}>
